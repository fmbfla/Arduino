...
void loop()
{
  ...
  if(Serial.available() >0){
    ...
    delay(10);
    if (Serialchar=='s'){//read the Status of the relays on request
      if (digitalRead(4)==LOW){
        Serial.println(" Bilge is OFF! ");
      }
      else Serial.println("Bilge is ON");
    }
    if (Serialchar=='s'){//read the Status of the relays on request
      if (digitalRead(5)==LOW){
        Serial.println(" Relay #5 is OFF! ");
      }
      else Serial.println(" Relay #5 is ON");
    }
    if (Serialchar=='s'){//read the Status of the relays on request
      if (digitalRead(6)==LOW){
        Serial.println(" Relay #6 is OFF! ");
      }
      else Serial.println(" Relay #6 is ON");
    }
    ...
    if (Serialchar=='p'){
      if(digitalRead(10)==LOW){
        //if (digitalRead(10)==LOW){
        power_on();
        Serial.println(num);// Send message to handset to notify the relays are ready
        delay(100);
    ...
    if (Serialchar=='o'){
      ...
      if (digitalRead(10)==HIGH){
        ...
        /*mySerial.println("num);  // recipient's mobile number, in international format
         delay(100);
         temp();
         mySerial.println(" POWERED DOWN via SMS...");// message to send
         delay(100);
         mySerial.println((char)26); // End AT command with a ^Z, ASCII code 26
         delay(100);
         mySerial.println();*/
        //void power_off(); 
        mySerial.println("AT+CPOWD=1");//Power Down 1=Normal 0=Urgent
        //return;
      }
      //delay(10);
      if (Serialchar=='o'){
  ...
  while(mySerial.available() >0){
    ...
    if (inchar=='#'){
      ...
      if (inchar=='r'){
        ...
        delay(100);
        mySerial.println(incoming_char);  // recipient's mobile number, in international format
        delay(100);
        mySerial.println((char)26); // End AT command with a ^Z, ASCII code 26
        delay(100);
        mySerial.println(); 
      ...
      if (inchar=='a'){
        Serial.println(" System is still Active!!");
        mySerial.println(num);                                     // recipient's mobile number, in international format
        delay(100);
        temp();
        mySerial.println(" System is still Active!");// message to send
        delay(100);
        mySerial.println((char)26); // End AT command with a ^Z, ASCII code 26
        delay(100);
        mySerial.println();
      ...
      }
      {
        if (digitalRead(4)==LOW){
          Serial.println(num);// Send message to handset to notify the relays are ready
          Serial.println(" Bilge Is CURRENTLY OFF!");
          delay(100);
          mySerial.println(num);  // recipient's mobile number, in international format
          delay(100);
          temp();
          mySerial.println(" Bilge Is CURRENTLY OFF!");// message to send
          delay(100);
          mySerial.println((char)26); // End AT command with a ^Z, ASCII code 26
          delay(100);
          mySerial.println(); 
        ...
        if (inchar=='s'){
          if (digitalRead(4)==HIGH){
            Serial.println(" BILGE IS ON!");
            Serial.println(num);// Send message to handset to notify the relays are ready
            delay(100);
            mySerial.println(num);  // recipient's mobile number, in international format
            delay(100);
            temp();
            mySerial.println(" BILGE IS STILL ON!");// message to send
            delay(100);
            mySerial.println((char)26); // End AT command with a ^Z, ASCII code 26
            delay(100);
            mySerial.println(); 
